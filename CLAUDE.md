...
- `scripts/check-publish-freshness.js` → `publish-freshness.json` — **publiserings-ferskhetsvakten (WP-248)**: henter den LIVE sidens `data/meta.json` (domene fra `docs/CNAME`) og dømmer den mot repoets — `stale` når live henger > 6 t bak (::error::-annotering, men feller ALDRI kjøringen: deployen er medisinen). Lukker klassen «alt innvendig grønt, siden råtner offentlig» (aug 2026: én Pages-deploy i «waiting» holdt `pages-deploy`-køen i fire uker; alle senere deploys ble kansellert bak den, og ingen andre signaler så det fordi alle dømmer repoet, ikke den publiserte kopien). self-repair leser signalet og rydder køen
...
4. **Calibration ledger** — verify appends one JSONL record per source check;
   ...
   the research agent's source choices — and since WP-245 bind them mechanically:
   `scripts/lib/calibration-gate.js` (brukt av build-events + validate-events)
   demoterer/avviser high-confidence-events hvis hele grunnlag er kilder med
   målt reliabilitet < 0.7.
...
`publish-freshness.json` (publiserings-ferskhetsvakten WP-248 — live-vs-repo-dommen per pipelinekjøring: `fresh`/`stale`/`unknown` + etterslep; les den i self-repair, git-historikken er tidslinjen),
`coverage-contracts.json` (dekningskontrakten WP-243 — det målbare løftet per wholesale-dekket konkurranse fra `authority.json`s `contract`-blokker, målt mot tavla hver kjøring: `met`/`breached`/`off-season` + telling; brudd blir `contract-breach`-gaps og farger `contracts`-porten i port-report),
...
- Client: `dashboard-cards` — loaded via `tests/helpers/load-client.js` (vm sandbox, no jsdom); `feed-vectors` (golden personalisation vectors, see `tests/fixtures/feed-vectors/DIVERGENCES.md`); `profile-shapes-board` (WP-200: a profile DERIVES `followBroadly` and sport-scopes the relevance blanket, so "kun Formel 1" gives kun F1 — an EMPTY profile reproduces the catalog-wide board id-for-id; Swift twin `ios/SportivistaTests/ProfileShapesBoardTests.swift`)
